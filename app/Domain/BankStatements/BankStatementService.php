...
final class BankStatementService
{
    ...
    {
        $this->assertAdministrator($actor, $store->getUserId());
     ...
     */
    public function updateDraft(BankStatement $statement, array $rows, User $actor): void
    {
        DB::transaction(function () use ($statement, $rows, $actor): void {
            $this->lockActiveStore($statement->getStoreId());
            $statement = $this->lockedStatement($statement);
     ...
     */
    public function retry(BankStatement $statement, User $actor): bool
    {
        $statement = DB::transaction(function () use ($statement, $actor): BankStatement {
            $this->lockActiveStore($statement->getStoreId());
            $statement = $this->lockedStatement($statement);
            $this->assertAdministrator($actor, $statement->getUserId());
    ...
    /**
     * Fence and requeue active imports during deployment with workers stopped.
     *
     * @return array{queued: int, failed: int}
     */
    public function requeueActiveImports(): array
    {
        $result = ['queued' => 0, 'failed' => 0];
        foreach (BankStatement::query()->whereIn('status', [BankStatementStatusEnum::QUEUED->value, BankStatementStatusEnum::PROCESSING->value])->lazyById(100) as $candidate) {
            $statement = DB::transaction(function () use ($candidate): BankStatement|null {
                $statement = $this->lockedStatement($candidate);
                if (!\in_array($statement->getStatus(), [BankStatementStatusEnum::QUEUED, BankStatementStatusEnum::PROCESSING], true) ||
                    $statement->getParseGeneration() !== $candidate->getParseGeneration()) {
                    return null;
                }
                $statement->update([
                    'status' => BankStatementStatusEnum::QUEUED->value,
            ...
            if ($statement instanceof BankStatement) {
                ++$result[$this->dispatchParsing($statement) ? 'queued' : 'failed'];
            }
        }
    ...
    public function fail(BankStatement $statement, string $error, int $generation, string|null $staleBefore = null): void
    {
        DB::transaction(function () use ($statement, $error, $generation, $staleBefore): void {
            ...
            if ($staleBefore !== null && !BankStatement::query()->whereKey($statement->getKey())
                ->where('status', BankStatementStatusEnum::PROCESSING->value)
                ->where('started_at', '<=', $staleBefore)->exists()) {
                return;
            }
    ...
    /**
     * Require the owning administrator for user-initiated bank mutations.
     */
    private function assertAdministrator(User $actor, int $ownerId): void
    {
        if (!$actor->isAdmin() || $ownerId !== $actor->getKey()) {
            \abort(404);
        }
    }
    ...
    {
        try {
            \dispatch(new ParseBankStatementJob($statement->getKey(), $statement->getParseGeneration()));
        } catch (Throwable) {
            $this->fail($statement, 'queue_dispatch_failed', $statement->getParseGeneration());
