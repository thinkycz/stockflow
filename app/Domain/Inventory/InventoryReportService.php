...
class InventoryReportService
{
    ...
    public function __construct(
        private readonly InventoryReadService $inventoryService,
    ) {
    }
    ...
    public function build(User $user, Store $store, Carbon $start, Carbon $cutoff): array
    {
        ...
        foreach ($storeItems as $storeItem) {
            ...
            if ($prediction['status'] === InventoryReadService::STATUS_SOON) {
                ++$risk['due_soon'];
            } elseif ($prediction['status'] === InventoryReadService::STATUS_OUT) {
                ++$risk['out'];
            } elseif ($prediction['status'] === InventoryReadService::STATUS_NO_DATA) {
                ++$risk['no_data'];
            }
            if ($prediction['coverage_days'] >= InventoryReadService::MINIMUM_COVERAGE_DAYS) {
