...
\test('a unique amount match conflicting with the calendar requires review without writes', function (): void {
    ...
    \expect($result['rows'][0]['automatic'])->toBeNull()
        ->and($result['rows'][0]['discovery_reason'])->toBe('estimate_only')
...
\test('near exact Wolt alternatives remain ambiguous rather than picking the closest', function (): void {
    ...
    \missingPayout($bank, ['amount' => '600.00']);
    $row = (new BankStatementReconciliationService())->forStatement($bank)['rows'][0];
    \expect($row['automatic'])->toBeNull()->and($row['discovery_reason'])->toBe('estimate_only')
...
\test('explicit labelled dates remain the priority suggestion despite an amount difference', function (): void {
    ...
    $row = (new BankStatementReconciliationService())->forStatement($bank)['rows'][0];
    \expect($row['automatic'])->toMatchArray(['source' => 'explicit'])->and($row['candidates'][0])->toMatchArray(['source' => 'explicit', 'from' => '2026-06-01', 'within_tolerance' => false, 'difference' => '-164.40']);
});
...
\test('a unique Wolt calendar estimate needs manual selection and candidates are deduplicated', function (): void {
    [$bank, $statement] = \payoutFixture();
    ...
    $row = (new BankStatementReconciliationService())->forStatement($bank)['rows'][0];
    \expect($row['automatic'])->toBeNull()->and($row['candidates'][0])->toMatchArray(['source' => 'calendar', 'from' => '2026-08-01', 'to' => '2026-08-05'])
...
\test('competing explicit Wolt periods preserve conflict diagnostics', function (): void {
    [$bank, $statement] = \payoutFixture();
    StatementDay::factory()->for($statement, 'statement')->create(['date' => '2026-08-01', 'wolt' => '1000']);
    foreach (['2026-08-07', '2026-08-08'] as $booked) {
        \missingPayout($bank, ['booked_on' => $booked, 'amount' => '600', 'description' => 'Settlement period: 1.8.2026 - 1.8.2026']);
    }
    foreach ((new BankStatementReconciliationService())->forStatement($bank)['rows'] as $row) {
