...
test('admin adjusts closes and prints payroll while limited users are denied', async ({
    ...
    await expect(e2eWorkerRow.locator('td').nth(3)).toContainText('400,00 Kč');
    ...
    await expect(payrollTotals.locator('td').nth(1)).toContainText('600,00 Kč');
    await expect(payrollTotals.locator('td').nth(2)).toContainText('600,00 Kč');
    await expect(payrollTotals.locator('td').nth(3)).toContainText('0,00 Kč');
    await expect(payrollTotals.locator('td').nth(4)).toContainText(
        '1 200,00 Kč',
    );
